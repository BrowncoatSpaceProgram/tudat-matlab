...
5. You will be prompted to provide the path to the `tudatBundle` folder you downloaded in step 1. You can skip this step by typing `ctrl + C` if you are planning to use tudat-matlab only in IO mode (see [Usage modes](#usage-modes)). You can specify `tudatBundle` binary path later by calling `tudat.find('tudatBundlePath')` from MATLAB's Command Window.
